...
function reWrite() {
    ...
    $.each(data, function (index, val) {
        if (state == 'wait') {//如果是未完成的
            ...
            if (val.isDone == 1) {
                str += "<li id='" + index + "'   style='background:#CCCCCC'><input type='checkbox'><time>" + getYear(val.time) + "<br><span>" + gettime(val.time) + "</span></time> <p>" + val.con + "</p><span class='iconfont '>&#xe662;</span><div class='del'>删除除</div></li>";
            }
        }
    })
    $('.content ul').html(str);
